#import "EasyPermissionBluetooth.h"
#import <CoreBluetooth/CoreBluetooth.h>



//@interface EasyPermissionBluetooth()<CBPeripheralManagerDelegate,CBCentralManagerDelegate>
@interface EasyPermissionBluetooth()<CBPeripheralManagerDelegate>
...
@implementation EasyPermissionBluetooth

+ (instancetype)sharedManager
{
    static EasyPermissionBluetooth* _sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[EasyPermissionBluetooth alloc] init];
    });
...
{
    [EasyPermissionBluetooth sharedManager].onComletion = nil;
    ...
    */
    ...
    [EasyPermissionBluetooth sharedManager].onCompletionState = completion;
    [[EasyPermissionBluetooth sharedManager]startBluetootch:show];
}
...
+ (void)authorizeMonitorWithState:(void(^)(BOOL granted,NSInteger state))onState
{
    if (onState)
    {
        [EasyPermissionBluetooth sharedManager].onState = onState;
        
        [EasyPermissionBluetooth sharedManager].bluetoothMonitor =  [[CBPeripheralManager alloc]initWithDelegate:[EasyPermissionBluetooth sharedManager] queue:nil options:@{CBCentralManagerOptionShowPowerAlertKey:@(false)}];
    }
    else if( [EasyPermissionBluetooth sharedManager].bluetoothMonitor )
    {
        [EasyPermissionBluetooth sharedManager].bluetoothMonitor = nil;
    }
}
...
+ (void)authorizeWithCompletion:(void(^)(BOOL granted,BOOL firstTime))completion
{
    ...
    [EasyPermissionBluetooth sharedManager].onCompletionState = nil;
    /*
     0:未确定
     1:限制
     2:拒绝
    ...
    */
    ...
    [EasyPermissionBluetooth sharedManager].onComletion = completion;
    [[EasyPermissionBluetooth sharedManager]startBluetootch:NO];
}
