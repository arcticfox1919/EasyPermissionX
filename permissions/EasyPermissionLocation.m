#import "EasyPermissionLocation.h"
...
@interface EasyPermissionLocation()<CLLocationManagerDelegate>
@property(nonatomic,strong) CLLocationManager *locationManager;
@property (nonatomic, copy) void (^permissionCompletion)(BOOL granted,BOOL firstTime);
@property (nonatomic, assign) BOOL firstTime;
@end

@implementation EasyPermissionLocation

+ (instancetype)sharedManager
{
    static EasyPermissionLocation* _sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[EasyPermissionLocation alloc] init];
    });
...
+ (BOOL)authorized
{
    if (@available(iOS 14,*))
    {
        ...
        CLAuthorizationStatus authorizationStatus = [[EasyPermissionLocation sharedManager].locationManager authorizationStatus];
...
{
    CLAuthorizationStatus authorizationStatus = [CLLocationManager authorizationStatus];
    [EasyPermissionLocation sharedManager].firstTime = NO;
    switch (authorizationStatus) {
        ...
        case kCLAuthorizationStatusNotDetermined:
        {
            ...
            [EasyPermissionLocation sharedManager].firstTime = YES;

            [[EasyPermissionLocation sharedManager]startGps:completion];
        }
            break;
